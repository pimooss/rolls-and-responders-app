...
import { getScenario, getScenarioList, DIFFICULTY_TABLE } from './scenarios';
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-400 text-xl max-w-lg mx-auto">
          Interactive Tabletop Exercise Tool
        </p>
      </div>

      {/* Scenario Selector */}
      <div className="w-full max-w-2xl">
        <label className="block text-slate-400 text-sm font-bold mb-2">Select Scenario</label>
        <select
      ...
      <div className="grid md:grid-cols-2 gap-6 w-full max-w-2xl">
        ...
        >
          <div className="flex items-center space-x-3 text-orange-500">
            <Terminal size={32} />
            <span className="font-bold text-xl">Facilitator Console</span>
          </div>
          <p className="text-slate-400">Control the scenario, see hidden background info, trigger injects, and manage the game flow.</p>
        </button>
        ...
        >
          <div className="flex items-center space-x-3 text-blue-500">
            <Monitor size={32} />
            <span className="font-bold text-xl">Player Display</span>
          </div>
          <p className="text-slate-400">Launch this view on a shared screen. Shows public logs, dice rolls, and current scenario status.</p>
        </button>
      </div>

      <p className="text-slate-600 text-sm mt-8">
        Tip: Open this URL in two separate tabs/windows to use both modes simultaneously.
      </p>
    </div>
  );
}
...
function FacilitatorView({ scenario, state, updateState, addLog, exit }) {
  ...
  const handleNextTurn = () => {
    if (state.turnIndex < scenario.turns.length - 1) {
      ...
      const phaseLog = {
        ...
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        type: 'phase',
        text: `Advanced to ${nextTurn.title}`
      };
  ...
  const handleRoll = (type = 'normal') => {
    ...
    if (type === 'advantage') {
      ...
      details = `Rolled ${d1} & ${d2} (Took Higher)`;
    } else if (type === 'disadvantage') {
      final = Math.min(d1, d2);
      details = `Rolled ${d1} & ${d2} (Took Lower)`;
    }

    const rollData = { value: final, type, details, timestamp: Date.now() };
    updateState({ lastRoll: rollData });
    addLog({ type: 'roll', text: `Rolled ${final} (${type})`, roll: rollData });
  };
  ...
  const resetGame = () => {
    if (confirm('Reset the entire game? This will clear all progress and logs.')) {
      updateState({
        turnIndex: 0,
        publicLog: [],
        lastRoll: null,
    ...
    <div className="flex flex-col h-screen">
      ...
      <header className="bg-slate-950 border-b border-slate-800 p-4 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <div className="bg-orange-600 text-white px-2 py-1 rounded text-xs font-bold uppercase tracking-wide">Facilitator</div>
          <h2 className="font-bold text-slate-200">{scenario.title}</h2>
        </div>
        <div className="flex items-center space-x-4">
           <span className="text-slate-500 text-sm">Turn {state.turnIndex + 1}/{scenario.turns.length}</span>
           <button onClick={resetGame} className="text-xs text-yellow-600 hover:text-yellow-400 underline">Reset Game</button>
           <button onClick={exit} className="text-xs text-slate-600 hover:text-white underline">Exit</button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col border-r border-slate-800 overflow-y-auto">
          ...
          <div className="p-6 border-b border-slate-800 bg-slate-900/50">
            <div className="flex justify-between items-end mb-4">
              <div>
                <h3 className="text-orange-400 text-sm uppercase font-bold tracking-wider mb-1">Current Phase</h3>
              ...
              >
                Next Turn <SkipForward size={16} className="ml-2" />
              </button>
            </div>
            
            <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 mb-4">
              <span className="text-xs font-bold text-blue-400 uppercase mb-2 block">Read to Players</span>
              ...
              >
                <MessageSquare size={12} className="mr-1" /> Re-send to Player Log
              </button>
            </div>
            ...
            <div className="bg-orange-950/30 p-4 rounded-lg border border-orange-900/50">
              <span className="text-xs font-bold text-orange-400 uppercase mb-2 flex items-center">
                <Shield size={12} className="mr-1" /> Facilitator Eyes Only
              </span>
          ...
          {/* Injects */}
          <div className="p-6">
            <h3 className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-4">Available Injects</h3>
        ...
        <div className="w-1/2 flex flex-col bg-slate-925">
          ...
          <div className="p-6 border-b border-slate-800">
             <h3 className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-4">Action Resolution</h3>
             <div className="grid grid-cols-3 gap-4 mb-6">
                <button onClick={() => handleRoll('normal')} className="flex flex-col items-center justify-center p-4 bg-slate-800 hover:bg-slate-700 rounded-xl border border-slate-700 transition-colors">
                  ...
                  <span className="font-bold text-white">Normal Roll</span>
                </button>
                <button onClick={() => handleRoll('advantage')} className="flex flex-col items-center justify-center p-4 bg-slate-800 hover:bg-green-900/30 border border-slate-700 hover:border-green-500/50 rounded-xl transition-colors group">
                  ...
                  <span className="font-bold text-green-500">Advantage</span>
                  <span className="text-xs text-green-500/60">Roll 2, Take High</span>
                </button>
                <button onClick={() => handleRoll('disadvantage')} className="flex flex-col items-center justify-center p-4 bg-slate-800 hover:bg-red-900/30 border border-slate-700 hover:border-red-500/50 rounded-xl transition-colors group">
                  ...
                  <span className="font-bold text-red-500">Disadvantage</span>
                  <span className="text-xs text-red-500/60">Roll 2, Take Low</span>
                </button>
             </div>
          ...
          <div className="flex-1 flex flex-col p-6 overflow-hidden">
             <h3 className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-2">Player Screen Log Preview</h3>
             <div className="flex-1 bg-black/40 rounded-lg border border-slate-800 overflow-y-auto p-4 font-mono text-sm space-y-3">
               {state.publicLog.length === 0 && <span className="text-slate-600 italic">No events yet...</span>}
    ...
    <div className="h-screen flex flex-col bg-slate-950 text-white overflow-hidden">
      ...
      <div className="bg-slate-900 border-b border-orange-500/30 p-6 flex justify-between items-center shadow-lg z-10">
         <div>
            ...
            <div className="flex items-center space-x-4 text-sm font-mono text-slate-400">
              ...
              <span className="flex items-center text-orange-500"><AlertTriangle size={14} className="mr-1" /> INCIDENT ACTIVE</span>
            </div>
         </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {state.activeInject && (
            <div className="mb-8 bg-red-950/40 border-l-4 border-red-500 p-6 rounded-r-lg animate-pulse">
               <div className="flex items-center text-red-500 font-bold uppercase tracking-widest mb-2">
                 <AlertTriangle className="mr-2" /> Critical Update
               </div>
          ...
          {state.lastRoll && (
             <div key={state.lastRoll.timestamp} className="mb-8 p-6 bg-slate-900/80 border border-slate-700 rounded-xl flex items-center justify-between animate-in slide-in-from-top-4 duration-500">
                <div className="flex items-center">
                   ...
                   <div>
                     <div className="text-slate-400 text-sm uppercase font-bold">Action Roll Result</div>
                ...
                <div className="flex space-x-2 text-xs font-mono text-slate-500">
                   <div className={state.lastRoll.value >= 5 ? "text-green-500 font-bold" : "opacity-30"}>Routine(5+)</div>
                   <div className={state.lastRoll.value >= 10 ? "text-yellow-500 font-bold" : "opacity-30"}>Chall(10+)</div>
                   <div className={state.lastRoll.value >= 15 ? "text-red-500 font-bold" : "opacity-30"}>Hard(15+)</div>
                </div>
             </div>
          )}
          ...
          {state.publicLog.length === 0 && (
            <div className="h-full flex items-center justify-center text-slate-700">
               <div className="text-center">
                 ...
                 <p className="text-xl font-light">Waiting for facilitator updates...</p>
               </div>
            </div>
          )}
        </div>
        ...
        <div className="w-80 bg-slate-900 border-l border-slate-800 p-6 hidden lg:block">
           <h3 className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-6 border-b border-slate-800 pb-2">Reference Data</h3>
              ...
              <div>
                ...
                <div className="space-y-3 relative pl-4 border-l-2 border-slate-800">
                   {scenario.turns.map((t, i) => (
                     <div key={t.id} className={`text-sm transition-all ${i === state.turnIndex ? 'text-white font-bold translate-x-1' : 'text-slate-600'}`}>
                       {t.title}
                       {i === state.turnIndex && <span className="block text-xs font-normal text-orange-500 mt-1">Current Phase</span>}
                     </div>
                   ))}
                </div>
              </div>
  ...
  if (log.type === 'inject') {
     ...
     return (
       <div className="p-4 rounded bg-red-950/20 border border-red-900/30 text-red-200/60 text-sm">
          <span className="font-bold text-red-500 uppercase text-xs block mb-1">Past Inject</span>
          {log.text}
       </div>
     );
  }
  ...
  if (log.type === 'roll') {
    return (
       <div className="flex items-start space-x-3 text-sm text-slate-400 py-2">
          ...
          <div>
            <span className="block font-mono text-slate-300">Action Roll: {log.roll.value}</span>
